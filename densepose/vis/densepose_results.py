...
class DensePoseResultsVisualizer(object):
    def visualize(
        ...
        for i, result in enumerate(densepose_result):
            ...
            # self.visualize_iuv_arr(context, iuv_array.cpu().numpy(), boxes_xywh[i]) # 由此进入，目前来看是改变context的值
            context = self.visualize_iuv_arr(context, iuv_array.cpu().numpy(), boxes_xywh[i])
...
class DensePoseMaskedColormapResultsVisualizer(DensePoseResultsVisualizer):
    ...
    #     mask[segm > 0] = 1
    #     image_bgr = self.mask_visualizer.visualize(image_bgr, mask, matrix, bbox_xywh) # 由此进入

    def visualize_iuv_arr(self, context, iuv_arr: np.ndarray, bbox_xywh):
        image_bgr = self.get_image_bgr_from_context(context)
        matrix = self.data_extractor(iuv_arr)
        segm = self.segm_extractor(iuv_arr)
        mask = np.zeros(matrix.shape, dtype=np.uint8)
